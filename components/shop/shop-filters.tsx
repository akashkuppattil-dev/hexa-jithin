    ...
    <div className="space-y-4">
      <Accordion type="multiple" defaultValue={["categories", "brands", "availability"]} className="w-full">
        ...
        <AccordionItem value="categories" className="border-none">
          <AccordionTrigger className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground hover:text-[#09757a] py-3 hover:no-underline select-none">
            Categories
          </AccordionTrigger>
              ...
              {categories.map((category) => (
                <div key={category.id} className="flex items-center space-x-2.5 group cursor-pointer" onClick={() => toggleCategory(category.id)}>
                  ...
                  <Label
                    ...
                    className={`text-[11px] font-medium cursor-pointer transition-colors ${selectedCategories.includes(category.id) ? "text-[#09757a] font-semibold" : "text-muted-foreground group-hover:text-foreground"}`}
                  >
                    {category.name}
                  </Label>
                </div>
        ...
        <AccordionItem value="brands" className="border-none">
          ...
          <AccordionContent className="pb-3 overflow-hidden">
            <div className="relative mb-3 group">
              ...
              <Input
                placeholder="Search..."
                className="h-8 pl-9 text-[11px] bg-background border-border focus:bg-accent/5 rounded-lg transition-all font-medium text-foreground placeholder:text-muted-foreground"
                value={brandSearch}
            ...
            <div className="space-y-2 pt-0.5 max-h-[180px] overflow-y-auto pr-2 custom-scrollbar">
              {filteredBrands.map((brand) => (
                <div key={brand} className="flex items-center space-x-2.5 group cursor-pointer" onClick={() => toggleBrand(brand)}>
                  ...
                  <Label
                    htmlFor={`brand-${brand}`}
                    className={`text-[11px] font-medium cursor-pointer transition-colors ${selectedBrands.includes(brand) ? "text-[#09757a] font-semibold" : "text-muted-foreground group-hover:text-foreground"}`}
                  >
                    {brand}
                  </Label>
                </div>
              ...
              {[
                ...
                <div key={option.id} className="flex items-center space-x-2.5 group cursor-pointer" onClick={() => toggleAvailability(option.id)}>
                  <Checkbox
                    ...
                    className="h-3.5 w-3.5 rounded border-border data-[state=checked]:bg-[#09757a] data-[state=checked]:border-[#09757a] transition-all"
                  />
                  <Label htmlFor={option.id} className={`text-[11px] font-medium cursor-pointer transition-colors ${availability.includes(option.id) ? "text-[#09757a] font-semibold" : "text-muted-foreground group-hover:text-foreground"}`}>
                    {option.label}
                  </Label>
                </div>
              ))}
